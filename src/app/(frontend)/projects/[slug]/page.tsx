    ...
    <article className="project-page container">
      <header className="project-page__header">
        ...
        <div className="project-page__meta">
          {project.role && <span className="project-page__meta-item">{project.role}</span>}
          {project.client && <span className="project-page__meta-item">{project.client}</span>}
          {project.timeline && <span className="project-page__meta-item">{project.timeline}</span>}
          {project.year && <span className="project-page__meta-item">{project.year}</span>}
          {project.projectURL && (
            <Link className="project-page__visit-link" href={project.projectURL} rel="noreferrer" target="_blank">
              Visit Site
            </Link>
          )}
        </div>
      ...
      {project.sections && project.sections.length > 0 && (
        <section className="project-page__sections">
          {project.sections.map((section) => (
            <article className="project-page__section" key={section.id || section.heading}>
              ...
              {section.subheading && <p className="project-page__section-subheading">{section.subheading}</p>}
